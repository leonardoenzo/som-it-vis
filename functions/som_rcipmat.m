% SOM representative Cross Information Potential (rCIP) matrix
%
% PROGRAM DESCRIPTION
% This function generates SOM rCIP-vis (heatmap).
%
% DATA & FUNCTION DICTIONARY
% Inputs: 
% sMap - SOM structure
% mu - matrix containing means associated with each neurons
% SIGMA_neurons - covariance matrix for each subset of the data associated
% with SOM neurons
...
function M = som_rcipmat(sMap, mu, SIGMA_neurons, conn)
    ...
    for i=1:a 
        for j=1:b
            if (j<b) % i=1:a , j=1:b-1 : Scan Columns (horizontal connections)
                ...
                M(2*i-1,2*j) = CIP(mu(ind1,:), mu(ind2,:), SIGMA_neurons(:,:,ind1), SIGMA_neurons(:,:,ind2));
            end  
            if (i<a) % j=1:b , i=1:a-1 : Scan Rows (vertical connections)              
                ind1 = som_sub2ind([a b],[i j]);
                ind2 = som_sub2ind([a b],[i+1 j]);
                M(2*i,2*j-1) = CIP(mu(ind1,:), mu(ind2,:), SIGMA_neurons(:,:,ind1), SIGMA_neurons(:,:,ind2));
            end
            if ((i<a)&&(j<b)) % i=1:a-1 , j=1:b-1 : Scan Diagonals (principal / secondary) 	               
                ...
                val_prin_diag = CIP(mu(ind1,:), mu(ind2,:), SIGMA_neurons(:,:,ind1), SIGMA_neurons(:,:,ind2));
                val_sec_diag = CIP(mu(ind3,:), mu(ind4,:), SIGMA_neurons(:,:,ind3), SIGMA_neurons(:,:,ind4));
